...
fun MaterialButton.setMaterialButton(
    content: String?, condition: Boolean, trueRes: Int, falseRes: Int
) {
...
fun ImageView.setImageUrl(imageUrl: String?, isCircle: Boolean = false, radius: Int = 0) {
    ...
    visibility = View.VISIBLE
    val builder = Glide.with(this).load(imageUrl).override(measuredWidth, measuredHeight)
...
fun ImageView.load(imageUrl: String, callback: (Bitmap) -> Unit) {
    Glide.with(this).asBitmap().load(imageUrl).override(measuredWidth, measuredHeight)
        .diskCacheStrategy(DiskCacheStrategy.ALL)
...
fun ImageView.setBlurImageUrl(blurUrl: String, radius: Int) {
    Glide.with(this).load(blurUrl).override(measuredWidth, measuredHeight)
        .diskCacheStrategy(DiskCacheStrategy.ALL).override(radius)
        .transform(BlurTransformation()).dontAnimate()
